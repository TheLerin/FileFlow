...
export default function ExcelToJsonPage() {
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                // Extract the first sheet's data to an array of objects
                const firstSheetName = workbook.SheetNames[0];
                const worksheet = workbook.Sheets[firstSheetName];
                const jsonArray = utils.sheet_to_json(worksheet, { defval: null });

                const baseName = file.name.replace(/\.[^/.]+$/, "");
                const jsonString = JSON.stringify(jsonArray, null, 2);
                const blob = new Blob([jsonString], { type: "application/json" });

                downloadFileBlob(blob, `${baseName}.json`);
            }
    ...
    return (
        <div className="container mx-auto max-w-4xl py-12 px-4">
            <h1 className="text-3xl font-bold text-gray-900 mb-4">Excel to JSON</h1>
            <p className="text-gray-600 mb-8">Convert grid data rows from an Excel file into a structured API-ready JSON document.</p>
