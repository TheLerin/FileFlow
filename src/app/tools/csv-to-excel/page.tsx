...
export default function CsvToExcelPage() {
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                // SheetJS auto-detects CSV formatting internally if initialized as array
                const workbook = read(arrayBuffer, { type: "array" });

                // Generate the new XLSX binary payload
                ...
                downloadFileBlob(blob, `${baseName}.xlsx`);
            }

        } catch (err: any) {
            console.error(err);
            setError("An error occurred. Ensure the selected files are valid raw CSV files.");
        } finally {
    ...
    return (
        <div className="container mx-auto max-w-4xl py-12 px-4">
            <h1 className="text-3xl font-bold text-gray-900 mb-4">CSV to Excel</h1>
            <p className="text-gray-600 mb-8">Wrap and convert messy CSV data blocks safely into the modern Excel (.xlsx) format.</p>

            <FileUploadArea onFileSelect={setFiles} multiple accept=".csv,text/csv" />
            ...
            {files.length > 0 && (
                <div className="mt-8 p-6 bg-white rounded-xl shadow-sm border border-gray-200 w-full max-w-2xl">
                    ...
                    >
                        {isProcessing ? <Loader2 className="w-5 h-5 animate-spin" /> : <Table2 className="w-5 h-5" />}
