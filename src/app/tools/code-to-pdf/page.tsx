...
export default function CodeToPdfPage() {
    const [code, setCode] = useState<string>("def calculate_fibonacci(n):\n    if n <= 1:\n        return n\n    else:\n        return calculate_fibonacci(n-1) + calculate_fibonacci(n-2)\n\nprint(calculate_fibonacci(10))");
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            // Rasterize the DOM node
            const canvas = await html2canvas(previewRef.current, {
                scale: 2, // High DPI for crisp text inside PDF
                useCORS: true,
            ...
            // Calculate layout dimension for PDF A4 Format
            ...
            // Dump to Blob array directly from jsPDF
            const blob = pdf.output('blob');
            downloadFileBlob(blob, `code-export_${Date.now()}.pdf`);

        } catch (err: any) {
            console.error(err);
            setError("An error occurred generating the document.");
        } finally {
    ...
    return (
        <div className="container mx-auto max-w-5xl py-12 px-4">
            <h1 className="text-3xl font-bold text-gray-900 mb-4">Code to PDF</h1>
            <p className="text-gray-600 mb-8">Export beautifully formatted code specifically wrapped for A4 PDF documentation natively in the browser.</p>
                ...
                <div className="w-full lg:w-1/3 flex flex-col gap-6">
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col gap-4">
                        ...
                        <button
                            ...
                            className={`w-full py-3 mt-2 rounded-lg font-medium text-white transition-all flex items-center justify-center gap-2 ${isProcessing || !code.trim() ? "bg-gray-400 cursor-not-allowed" : "bg-[#EA4335] hover:bg-[#D53F32]"
                                }`}
                        >
                            {isProcessing ? <Loader2 className="w-5 h-5 animate-spin" /> : <FileText className="w-5 h-5" />}
                            {isProcessing ? "Rendering..." : "Export as PDF"}
                        </button>
                    </div>
                </div>
